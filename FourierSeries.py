...
def main():
    ...
    duty_ratio_slider.on_changed(update_duty_ratio(fig, duty_ratio_slider, time_lines, markerline, stemlines, y_text_arr))
    reset_button.on_clicked(reset(duty_ratio_slider))
    show_or_hide_button.on_clicked(show_or_hide(fig, y_text_arr, show_or_hide_button))
...
# Functions
def update_duty_ratio(fig, slider, time_lines, markerline, stemlines, y_text_arr):
    def funct(event):
        time_x_arr, time_y_arr = get_data_time(slider.val)
        time_lines.set_xdata(time_x_arr)
        time_lines.set_ydata(time_y_arr)

        freq_x_arr, freq_y_arr = get_data_frequency(slider.val)
...
def show_or_hide(fig, y_text_arr, show_or_hide_button):
    def funct(event):
